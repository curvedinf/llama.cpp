#!/usr/bin/env bash
# Golden-reference generator for the divergence gate.
# Layer split keeps tensors whole, producing correct sequential output.
# Use with concurrent_correctness_gate.py --mode ref.
set -euo pipefail
...
COMMON_ARGS=(
  ...
  --threads-http 12
  -lv 5 -cb -ngl 999 -fit off --no-mmap
...
case "${1:-start}" in
  start)
    ...
    sleep 2
    log="${LOG_DIR}/server-golden-ref.log"
    setsid env \
      ...
      LD_LIBRARY_PATH="${LD_LIBRARY_PATH}" \
        ...
        >"${log}" 2>&1 </dev/null &
    echo "started golden-ref server pid=$! port=${PORT}"
    for _ in $(seq 1 240); do
      if curl -fsS --max-time 2 "http://${HOST}:${PORT}/health" 2>/dev/null | grep -q '"ok"'; then
        echo "golden-ref server ready"
        exit 0
      fi
      sleep 2
    done
    echo "golden-ref server failed to start"; tail -20 "${log}" >&2; exit 1
    ;;
  stop) pkill -f "llama-server.*--port ${PORT}" 2>/dev/null || true; sleep 2; echo "stopped" ;;
  *) usage; exit 1 ;;
esac
